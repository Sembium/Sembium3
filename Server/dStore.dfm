inherited dmStore: TdmStore
  ...
  object qryMovements: TAbmesSQLQuery
    ...
    SQL.Strings = (
      'select'
      '  /*+ LEADING(sd) INDEX(sd idxSD__STORE_DEAL_DATE) */'
      '  sd.STORE_CODE,'
      '  ( select'
      '      (dt.DEPT_TYPE_ABBREV || d.CUSTOM_CODE || d.SUFFIX_LETTER)'
      '    from'
      ...
      '    where'
      '      (p.PRODUCT_CODE = sd.PRODUCT_CODE)'
      '  ) as PRODUCT_COMPANY_PRODUCT_NAMES,'
      ''
      '  ( select'
      '      p.CUSTOM_CODE'
      '    from'
      ...
      ''
        ...
        '  (sd.STORE_DEAL_DATE between Trunc(:START_DATE) and Trunc(:END_' +
        'DATE)) and  -- use Trunc() to make the oprimizen not peek into t' +
        'he params'
      ''
      '  ( (:ALL_FILTERED_DEPTS = 1) or'
      '    (exists'
      ...
      '        DEPT_RELATIONS dr'
      '      where'
      '        (dr.DESCENDANT_DEPT_CODE = sd.DEPT_CODE) and'
      '        (dr.ANCESTOR_DEPT_CODE = :OTHER_DEPT_CODE)'
      '      )'
      '    )'
      '  ) and'
      ''
