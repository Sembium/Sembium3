unit uSQLScriptor.StatementExecutor;
...
type
  ...
  strict private
    ...
    FQuery: TAbmesSQLQuery;
    FQueryIntf: IQuery;
    procedure SetSqlConnectionParams(const ADBName: string);
    procedure DoConnect;
    procedure DoDisconnect;
  protected
    function GetQuery: IQuery; override;
  public
    constructor Create(const ADBName: string; const AConnectionNo: Integer; const ALogger: ILogger);
...
procedure TDBSqlStatementExecutor.ExecStatement(
  ...
  var
    p: TParam;
    SameParam: TEqualityComparison<TParam>;
  begin
    Result:= '';

    SameParam:=
      function(const A, B: TParam): Boolean
      begin
        Result:= (A.Name = B.Name);
      end;

    for p in AParams.All.Distinct(SameParam) do
      Result:=
        ConcatLines(
          Result,
...
constructor TDBXSqlStatementExecutor.Create(const ADBName: string; const AConnectionNo: Integer; const ALogger: ILogger);
var
  BeginDateTime: TDateTime;
begin
  BeginDateTime:= Now;
  try
    ...
    SetSqlConnectionParams(FDBName);
...
procedure TDBXSqlStatementExecutor.SetSqlConnectionParams(const ADBName: string);
var
  ObjParams: TStrings;
begin
  ObjParams:= TStringList.Create;
  try
    LoadObjParams(ObjParams, ADBName, DBObjParamNames, GetDBDataModulesConfigKey);
    uDBUtils.SetSqlConnectionParams(FSQLConnection, ObjParams);
  finally
    FreeAndNil(ObjParams);
  end;
end;

{ TDBXSqlStatementExecutorFactory }

constructor TDBXSqlStatementExecutorFactory.Create(const ADBName: string; const ALogger: ILogger);
begin
...
begin
  Result:= TDBXSqlStatementExecutor.Create(DBName, GetNewConnectionNo, Logger);
end;
