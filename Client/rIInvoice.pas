...
type
  TrptIInvoice = class(TDBDataReport)
    QRBand1: TQRBand;
    QRBand2: TQRBand;
    bndDetail: TQRBand;
    bntSummary: TQRBand;
    ...
    bntSummaryFooter: TQRChildBand;
    lblSellerPersonEmptyName: TQRLabel;
    lblBuyerPersonEmptyName: TQRLabel;
    QRLabel30: TQRLabel;
    QRLabel33: TQRLabel;
    QRLabel34: TQRLabel;
    dbtSellerPersonIName: TQRDBText;
    dbtBuyerPersonIName: TQRDBText;
...
class procedure TrptIInvoice.PrintReport(AReportDataSet,
...
procedure TrptIInvoice.QuickRepBeforePrint(Sender: TCustomQuickRep; var PrintReport: Boolean);
begin
  inherited;
  ...
  dbtBuyerPersonIName.Enabled:= not ParamsDataSet.FieldByName('BUYER_PERSON_I_NAME').IsNull;
  lblBuyerPersonEmptyName.Enabled:= ParamsDataSet.FieldByName('BUYER_PERSON_I_NAME').IsNull;

  dbtSellerPersonIName.Enabled:= not ParamsDataSet.FieldByName('SELLER_PERSON_I_NAME').IsNull;
  lblSellerPersonEmptyName.Enabled:= ParamsDataSet.FieldByName('SELLER_PERSON_I_NAME').IsNull;

  bndSummaryCurrencyVAT.Enabled:=
    (ParamsDataSet.FieldByName('CURRENCY_CODE').AsInteger <> LoginContext.BaseCurrencyCode);
end;

procedure TrptIInvoice.SetDataParams(AReportDataSet, AParamsDataSet: TDataSet; AIsDiscountPercentShown: Boolean);
begin
